module.exports = function(host, port, retry) {
	...
	priv.reserveReply = function(successCallback) {
		...
		priv.connect(function(err) {
			...
			priv.runReply(function() {
				...
				replyClient.watch(replyTube, priv.generateClientCallback(replyClient, function(err) {
					if (err) return callback(err);

					replyClient.reserve(priv.generateClientCallback(replyClient, function(err, jobid, payload) {
						if (err) return callback(err);
	...
	publ.reserve = function(tube, successCallback) {
		...
		priv.connect(function(err) {
			...
			priv.runWork(function() {
				...
				workClient.watch(tube, priv.generateClientCallback(workClient, function(err, numwatched) {
					if (err) return callback(err);

					workClient.reserve(priv.generateClientCallback(workClient, function(err, jobid, payload) {
						workRunning = false;
	...
	priv.generateClientCallback = function(client, callback) {
